...
class ActivityCategoryController(
    ...
    @PostMapping("/create")
    fun createActivityCategory(
        @RequestBody payload: CreateActivityCategoryDto,
        @RequestHeader(
            "Authorization"
        ) token: String
    ): String {
    ...
    @PostMapping("/delete")
    fun deleteActivityCategory(
        @RequestBody payload: DeleteActivityCategoryDto,
        @RequestHeader(
            "Authorization"
        ) token: String
    ): String {
        val user = tokenService.parseToken(token.substring("Bearer".length))
        val categoryToDelete = activityCategoryService.findByInstructorIdAndName(
            user.id!!,
            payload.name
        )
