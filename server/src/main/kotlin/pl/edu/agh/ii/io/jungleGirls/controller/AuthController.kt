...
class AuthController(
    ...
    fun login(@RequestBody payload: LoginDto): AuthResponseDto {
        when(val user = studentService.login(payload.username,payload.password)){
            is Either.Right -> {
    ...
    fun register(@RequestBody payload: RegisterDto): AuthResponseDto {
        if (studentService.findByUsername(payload.username)!=null) {
            throw ResponseStatusException(HttpStatus.BAD_REQUEST, "Name already exists")
        }
        val user = LoginUser(
        ...
        when(val savedUser = studentService.createUser(user)){
            is Either.Right -> {
                return AuthResponseDto(
