...
class ActivityService(
    ...
    fun createActivity(
        instructorId: Long,
        activity: Activity,
        groupIds: ArrayList<Long>,
        courseGroupStartDates: ArrayList<LocalDateTime>
    ): Either<String, None> {
                ...
                validateStartDates(courseGroupStartDates).flatMap { _ ->
                    ...
                    fun transaction(): Boolean {
                        ...
                        for ((groupId, startDime) in groupIds.zip(courseGroupStartDates)) {
                            courseGroupActivityRepository.save(
                                CourseGroupActivity(groupId, createdActivity.id!!, startDime)
                            ).block() ?: return false
                        }
                        return true
                    }
    ...
    private fun validateActivity(instructorId: Long, activity: Activity): Either<String, None> {
        return checkIsBlank(activity.name, "name can not be empty")
            .flatMap { _ ->
                checkIfNameIsNotTaken(instructorId, activity.name)
                    .flatMap { _ ->
                        checkIsBlank(activity.description, "description can not be empty")
                            .flatMap { _ ->
                                checkIsDurationPositive(activity.duration)
                                    .flatMap { _ ->
                                        checkIsMaxScorePositive(activity.maxScore)
                                    }
                            }
                    }
            }
    ...
    private fun validateStartDates(courseGroupStartDates: ArrayList<LocalDateTime>): Either<String, None> {
        return if (courseGroupStartDates.any { LocalDateTime.now().minusMinutes(1).isAfter(it) }) {
            "Activity can not start in the past!".left()
        } else {
            None.right()
        }
    }
